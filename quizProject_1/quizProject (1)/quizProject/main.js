...
const timer = setInterval(() => {
  --seconds;
  // if the timer count from 1:30 until 1:00
  // min will be min-1, secound start cowntdown from 59
  if (seconds < 0 && min > 0) {
    min -= 1; //min=min-1
    seconds = 59;
  }
  ...
  //To add Zero"0" before ones numbers "05"
...
/////
//////////////// remove previous question check
...
const getSelected = () => {
  let answer;
  //selected answers=answers id: (a,b,c,d)
...
const btnclick = btn.addEventListener("click", (e) => {
  ...
  if (answer) {
    if (answer === randomqustions[currentQuiz].correctOption) {
      ...
      score++;
      //collect correct answers in Array for the report
      correctAnsewrsArray.push(randomqustions[currentQuiz]);
      console.log(correctAnsewrsArray);
    } else {
      //collect wrong answers  in Array for the report
  ...
  //local storage for wrong and correct answers
...
////////////////Loud new question every next btn
const loadQuiz = () => {
  min = 1;
  seconds = 31;
  deselectAnswers();
